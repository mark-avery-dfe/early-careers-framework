  ...
  (1..4).each do |school_number|
    ...
    (2021..2023).each do |start_year|
      ...
      { "Mentor" => NewSeeds::Scenarios::Participants::Mentors::MentorWithNoEcts,
        ...
        # #{participant_type} Ineligible
        scenario_klass.new(school_cohort:, full_name: "#{participant_type} #{start_year} School #{school_number} Ineligible for funding")
                      .build(induction_start_date: nil)
                      .with_induction_record(induction_programme: school_cohort.default_induction_programme)
                      .with_validation_data
                      .with_eligibility(status: "ineligible", reason: "no_induction")

        # #{participant_type} Ineligible with only non-completed billable declaration
        scenario_klass.new(school_cohort:, full_name: "#{participant_type} #{start_year} School #{school_number} with non-completed billable declaration and Ineligible for funding")
                      .build
                      .with_induction_record(induction_programme: school_cohort.default_induction_programme)
                      .with_validation_data
